...
class QdTrainer(Trainer):
    def __init__(self, opt):
        self.opt = opt
        if self.opt["model"] == "QDCDR":
            self.model = QDCDR(opt)
        else :
    ...
    def unpack_batch(self, batch):
        if self.opt["cuda"]:
            ...
            share_pos_item = inputs[5]
            share_neg_item = inputs[6]
        else:
            inputs = [Variable(b) for b in batch]
            user = inputs[0]
            source_pos_item = inputs[1]
            ...
            share_pos_item = inputs[5]
            share_neg_item = inputs[6]
        return user, source_pos_item, source_neg_item, target_pos_item, target_neg_item, share_pos_item, share_neg_item
    ...
    def evaluate_embedding(self, source_UV, source_VU, target_UV, target_VU, share_UV, share_VU, source_adj=None, target_adj=None):
        self.source_user, self.source_item, self.target_user, self.target_item, self.share_user, self.share_item, self.mask_dict = self.model(source_UV, source_VU, target_UV, target_VU, share_UV, share_VU)
    ...
    def reconstruct_graph(self, batch, source_UV, source_VU, target_UV, target_VU, share_UV, share_VU, source_adj=None, target_adj=None, epoch = 100):
        ...
        user, source_pos_item, source_neg_item, target_pos_item, target_neg_item, share_pos_item, share_neg_item = self.unpack_batch(batch)

        # if epoch<10:
        #     self.source_user, self.source_item, self.target_user, self.target_item, self.share_user, self.share_item, self.mask_dict = self.model.wramup(source_UV,source_VU,target_UV,target_VU,share_UV,share_VU)
        # else:
        self.source_user, self.source_item, self.target_user, self.target_item, self.share_user, self.share_item, self.mask_dict = self.model(source_UV,source_VU, target_UV,target_VU,share_UV,share_VU)
        ...
        share_user_feature = self.my_index_select(self.share_user, user)
        share_item_pos_feature = self.my_index_select(self.share_item, share_pos_item)
        share_item_neg_feature = self.my_index_select(self.share_item, share_neg_item)
        ...
        pos_share_score = self.model.share_predict_dot(share_user_feature, share_item_pos_feature)
        neg_share_score = self.model.share_predict_dot(share_user_feature, share_item_neg_feature)

        pos_labels, neg_labels = torch.ones(pos_source_score.size()), torch.zeros(
            pos_source_score.size())
        if epoch > 10:
            # 计算范数
            norm_E1 = torch.norm(self.source_item, p='fro')
            norm_E2 = torch.norm(self.target_item, p='fro')
            # 计算范数约束损失
        ...
        loss = self.criterion(pos_source_score, pos_labels) + \
               ...
               self.criterion(pos_share_score, pos_labels) + \
               self.criterion(neg_share_score, neg_labels) + \
               self.model.source_specific_GNN.encoder[-1].kld_loss + \
               self.model.target_specific_GNN.encoder[-1].kld_loss + \
               self.model.kld_loss + norm_loss
